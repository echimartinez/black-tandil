...
export async function POST(req: Request) {
  try {
    ...
    const totalAmount = validatedItems.reduce((sum, item) => sum + item.product.price * item.quantity, 0);
    ...
    return new Response(JSON.stringify({ init_point: response.init_point }), { status: 200 });
  } catch (error: any) {
