  ...
  return (
    <div className="w-full pb-28">
            ...
            {images.map((_, i) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all bg-white ${
                  i === selectedImage ? "w-4 opacity-100" : "w-1.5 opacity-50"
                }`}
              />
            ))}
          </div>
      ...
      <div className="px-4 pt-5">
        ...
        <h1 className="font-bebas text-4xl tracking-tight text-[#111] mt-0.5 leading-none">
          {product.name}
        </h1>
        {product.description && (
          <p className="font-dm text-sm text-[#888] mt-1">{product.description}</p>
        )}
        <p className="font-dm font-bold text-2xl text-[#111] mt-3">
          ${product.price.toLocaleString("es-AR")}
        </p>
        <p className="font-dm text-xs text-[#888] mt-0.5">
      ...
      <div className="px-4 mt-6">
        <div className="flex items-center justify-between mb-3">
          <p className={`font-dm text-xs uppercase tracking-widest font-semibold transition-colors ${
            sizeError ? "text-[#E63A2E]" : "text-[#888]"
          }`}>
              ...
              <button
                ...
                className={`py-3 border rounded-sm font-dm font-semibold text-sm transition-all relative ${
                  selectedSize === size
                    ...
                    : outOfStock
                    ? "bg-[#F5F4F0] text-[#CCC] border-[#E0DED8] cursor-not-allowed line-through"
        ...
        {selectedSize && stockForSize !== null && (
          <p className="font-dm text-xs text-[#E63A2E] mt-2">
            Solo quedan {stockForSize} en nuestros almacenes
          </p>
        )}
      </div>
      ...
      {product.longDescription && (
        <div className="px-4 mt-6">
          <p className="font-dm text-sm text-[#444] leading-relaxed">
            {product.longDescription}
          </p>
        </div>
      )}
      ...
      {product.benefits && (
        <div className="px-4 mt-5">
          <p className="font-dm text-xs text-[#888] uppercase tracking-widest font-semibold mb-3">
            Beneficios
          </p>
        ...
        {[
          {
            ...
            content: "Hacemos envíos a todo el país por correo argentino o Andreani. Los cambios y devoluciones se aceptan dentro de los 10 días de recibido el producto, siempre que esté en su estado original con etiquetas.",
          },
          {
            key: "pagos",
            title: "Métodos de pago",
            content: "Aceptamos MercadoPago, tarjetas de crédito y débito de todos los bancos. Hasta 6 cuotas sin interés con tarjetas seleccionadas. También transferencia bancaria con 10% de descuento.",
          },
          ...
          <div key={key} className="border-b border-[#E0DED8]">
            ...
            {openSection === key && (
              <p className="font-dm text-sm text-[#666] pb-4 leading-relaxed">{content}</p>
            )}
          </div>
        ))}
      </div>
